...
def funnyString(s):
    ...
    r_ascii = [ord(j) for j in r]
    s_abso_diff = [abs(s_ascii[i]-s_ascii[i+1]) for i in range(len(s_ascii)-1)]
    r_abso_diff = [abs(r_ascii[j]- r_ascii[j+1]) for j in range(len(r_ascii)-1)]
